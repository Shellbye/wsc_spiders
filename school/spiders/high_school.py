...
class HighSchoolProfile(scrapy.Item):
    url = scrapy.Field()
...
class HighSchoolSpider(scrapy.Spider):
    name = "high_school"
    collection = "high_school"
    unique_key = 'url'
    allowed_domains = ["school.zhongkao.com"]
    start_urls = (
        'http://school.zhongkao.com/province/',
    )

    def parse(self, response):
        total_page = int(response.xpath("//nav[@class='page_Box tc']/a[10]/text()")[0].extract())
        for i in range(1, total_page + 1):
            link = "http://school.zhongkao.com/province/0/p" + str(i)
